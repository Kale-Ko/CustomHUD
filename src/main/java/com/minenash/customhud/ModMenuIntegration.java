...
public class ModMenuIntegration implements ModMenuApi {
    ...
    private static class ConfigScreen extends Screen {
        ...
        protected void init() {
            ...
            for (int i = 1; i <= 3; i++) {
                int ii = i;
                this.addDrawableChild(ButtonWidget.builder(MutableText.of(new TranslatableTextContent("config.custom_hud.open_profile",i)), (button) -> new Thread(() -> Util.getOperatingSystem().open(CustomHud.getProfilePath(ii).toFile())).start()).dimensions(mid - 75,OPTION_START + OPTION_BUFFER*(i+1) + 5,150,20).build());
